...
이 문서는 `StackPilot`가 회사 운영을 도울 때 작업 기록을 어떻게 남기고
다음 실행으로 이어갈지 정의한다.
...
4. `StackPilot`는 record contract와 provider entrypoint만 설치한다
...
active local task-state가 있으면 `stack-pilot record --from-task-state`로
...
`StackPilot`는 외부 tool의 runtime을 대신하지 않는다.

| 영역 | 권장 source of truth | `StackPilot` 역할 |
|---|---|---|
...
- `stack-pilot record` local docs 생성 명령
- `stack-pilot record --surface github-issue|both` GitHub issue 생성 명령
- `stack-pilot record --from-task-state` active task-state attach 및 owner/next_action fallback
...
stack-pilot internal task-state begin --title "Build auth flow" --phase execute --owner codex --next-action "capture resumable record"
...
stack-pilot record --type handoff --title "CRM setup" --surface github-issue --dry-run
```
...
최종 기준은 단순하다.
`StackPilot`는 일을 대신 저장하는 회사 DB가 아니라, 일을 이어서 처리할 수 있게
