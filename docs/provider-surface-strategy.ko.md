...
이 문서는 `StackPilot`가 멀티에이전트 하네스와 `gstack`식 workflow contract를
각 LLM의 native surface에 맞게 어떻게 배포할지 정리한다.

핵심 원칙은 하나다.

> 카탈로그와 workflow 의도는 공통으로 관리하되, 제품 표면은 provider별 native kit으로 나눈다.

즉 `plugin`, `extension`, `skill`, `command`, `mcp`를 모두 억지로 하나로 통일하지
않는다. 각 LLM이 잘 먹는 표면에 맞춰서 `codex-kit`, `gemini-kit`,
`claude-kit`으로 배포한다.

`StackPilot`는 이 세 kit의 umbrella installer, doctor, probe, migration
tool이다.
...
- Gemini는 `extensions/stackpilot-dev`, `settings.json`, command TOML을 중심으로 설치한다
...
즉 `StackPilot`는 provider native surface를 "대체"하지 않고
...
한 줄로 말하면 "공통 의도와 검증 1개, provider-native kit 3개"가 가장 가볍다.
