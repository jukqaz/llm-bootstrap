...
이 문서는 현재 `llm-bootstrap`와 공개 `oh-my-*` 시리즈를 같은 질문으로 비교한다.
...
- `llm-bootstrap`는 설치기이자 기준선 관리자다
- `oh-my-*`는 실행기이자 orchestration runtime이다
- 따라서 1:1 대체재 비교보다 "제품 계층이 어떻게 다르나"를 먼저 봐야 한다

## 한눈에 보는 분석표

| 항목 | `llm-bootstrap` | `oh-my-codex` | `oh-my-claudecode` | `oh-my-gemini` |
|---|---|---|---|---|
...
| 항목 | 평가 |
|---|---|
| `llm-bootstrap` 강점 | `Codex`, `Gemini`, `Claude`를 각자 native surface로 맞추면서도 공통 pack 구조를 유지한다 |
| `llm-bootstrap` 약점 | 바로 일을 "대신 굴리는" 첫 체감은 약하다 |
...
| 항목 | 평가 |
|---|---|
| `llm-bootstrap` 강점 | `ralph-loop`, `ralph-plan`, `delivery`, `incident`, `founder-loop`, `operating-review` 같은 공통 하네스를 preset과 pack에 넣었다 |
| `llm-bootstrap` 약점 | 멀티에이전트 runtime 자체를 강하게 소유하지 않는다. 실행 품질은 provider-native lane에 의존한다 |
...
| 항목 | 평가 |
|---|---|
| `llm-bootstrap` 강점 | Codex는 plugin/agent, Gemini는 extension/command, Claude는 skill/subagent doc/MCP 흐름으로 분리한다 |
| `llm-bootstrap` 약점 | 공통 UX가 약해서 provider마다 체감이 조금 다를 수 있다 |
| `oh-my-*` 강점 | 각 제품은 자기 주력 provider에서 매우 높은 체감을 만든다 |
| `oh-my-*` 약점 | 공통 모드와 런타임을 강하게 밀기 때문에, provider native UX를 완전히 존중하는 구조는 아니다 |

판단:

- provider를 오래 안정적으로 운영하려면 `llm-bootstrap` 쪽이 더 관리하기 쉽다
...
| 항목 | 평가 |
|---|---|
| `llm-bootstrap` 강점 | install/replace/uninstall/restore/doctor를 제품 핵심 계약으로 둔다 |
| `llm-bootstrap` 약점 | 런타임이 아니라 설치기이므로, 실행 중 관측 기능은 상대적으로 약하다 |
| `oh-my-*` 강점 | setup UX는 빠르다 |
| `oh-my-*` 약점 | 공개 README 기준으로는 home-state drift, backup, restore, uninstall 예측 가능성보다 runtime 사용성이 더 앞선다 |

판단:

- 홈 설정이 자주 꼬이는 환경에서는 `llm-bootstrap`가 더 강하다
...
| 항목 | 평가 |
|---|---|
| `llm-bootstrap` 강점 | `founder-pack`, `ops-pack`, `linear`, `gmail`, `calendar`, `drive`, `figma`, `stitch`, company automations 메타가 이미 있다 |
| `llm-bootstrap` 약점 | connector auth와 recurring scheduler는 아직 runtime-managed 경계 밖이다 |
| `oh-my-*` 강점 | 개발 실행력은 높다 |
...
| 항목 | 평가 |
|---|---|
| `llm-bootstrap` 강점 | 카탈로그와 renderer가 분리돼 있어 drift 관리가 상대적으로 쉽다 |
| `llm-bootstrap` 약점 | 기능 체감이 runtime 제품보다 느릴 수 있다 |
...
- `llm-bootstrap`는 `oh-my`의 대체품이 아니라, 더 아래층의 기반 레이어다
...
> `oh-my-*`는 빠르게 일하게 만드는 팀장이고, `llm-bootstrap`는 환경과 정책을 안 꼬이게 유지하는 운영 기반이다.
