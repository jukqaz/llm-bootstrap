...
따라서 `llm-bootstrap`는 이제 두 레이어로 나눈다.
...
```bash
llm-bootstrap baseline
llm-bootstrap add <capability>
llm-bootstrap remove <capability>
llm-bootstrap sync
llm-bootstrap doctor
llm-bootstrap wizard
```
...
- legacy `omx`, `omg`, `omc`, `oh-my-*` 파일과 env cleanup
...
```bash
llm-bootstrap add project
llm-bootstrap add growth
llm-bootstrap add support
llm-bootstrap add company
llm-bootstrap add solo-company
```
...
`llm-bootstrap`는 record contract와 provider별 entrypoint를 설치한다.
...
`llm-bootstrap`는 LLM별 baseline을 맞추는 설치기를 넘어서,
