...
`llm-bootstrap` 설계에 바로 영향을 주는 best-practice 아티클만 추려서 정리한다.
...
- MCP 출력이 커질 때는 warning threshold와 `MAX_MCP_OUTPUT_TOKENS`를 공식적으로 관리한다.
...
즉 `llm-bootstrap`가 따라야 할 공식 기준은 아래로 요약된다.
