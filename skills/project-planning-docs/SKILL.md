...
### 2) 질문 게이트 (필수)
- `references/question-bank.md`의 필수 질문을 먼저 묻고 답변을 받기 전에는 문서 작업을 시작하지 말아라.
- 필요 최소 질문만 하라. 선택 질문은 필수 답변 이후에만 묻는다.
- 답변이 부족하면 추가 질문을 이어가라.

### 3) 범위 결정
- `references/decision-criteria.md`로 라이트/표준/확장 범위를 결정하라.
- 모호하면 사용자에게 선택을 요청하라.
...
## 네이밍/대소문자 규칙
- 파일명은 `AGENTS.md`, `PRD.md`, `TRD.md`를 사용하라.
...
### references/
- `decision-criteria.md`: 범위 결정 기준.
- `question-bank.md`: 기획 질문 목록.
