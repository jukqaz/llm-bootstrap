...
|------|-------------|------|
| access-audit | 접근 권한과 역할 구성을 점검하고 변경 이력을 정리해야 할 때 사용. | `skills/access-audit` |
| agents-followup-docs | AGENTS.md에 참조된 PRD.md와 TRD.md를 프로젝트 규모 분기 기준으로 생성/갱신하고 링크를 정리하라. AGENTS.md 작성 이후 PRD/TRD가 필요할 때 사용. | `skills/agents-followup-docs` |
| agents-md | Create or update root and nested AGENTS.md by asking targeted questions and merging the answers into the existing files. Use when users want to bootstrap, regenerate, or refine AGENTS.md for single or monorepo projects. AGENTS.md 생성/갱신 요청이 있을 때 사용. | `skills/agents-md` |
| analytics-summary | 지표/분석 결과를 요약해 공유해야 할 때 사용. | `skills/analytics-summary` |
...
| deploy-checklist | 배포 직전/직후 체크 항목을 간단히 정리해야 할 때 사용. | `skills/deploy-checklist` |
...
| facilities-admin | 사무/시설 운영과 자산 관리 산출물을 정리해야 할 때 사용. | `skills/facilities-admin` |
| faq-builder | 자주 묻는 질문을 구조화하고 답변 템플릿을 작성해야 할 때 사용. | `skills/faq-builder` |
| feature-map-builder | 기능 목록과 모듈 관계를 정리해야 할 때 사용. | `skills/feature-map-builder` |
| finance-accounting | 예산/정산/매출 집계 등 재무/회계 산출물을 정리해야 할 때 사용. | `skills/finance-accounting` |
| github-pr-ci | GitHub PR 생성/업데이트/리뷰, CI/CD 상태 점검, 실패 원인 분석 및 재실행, 체크리스트 기반 품질 확인이 필요할 때 사용. GitHub MCP를 활용한 PR/Actions 상태 조회와 코멘트/리뷰 처리에 적합. | `skills/github-pr-ci` |
...
| legal-contracts | 계약/NDA/약관 등 법무 문서를 정리하거나 변경 기록을 관리해야 할 때 사용. | `skills/legal-contracts` |
...
| outsourcing-handoff | 외주/협력사에 작업 범위, 실행 절차, 산출물 기준을 인수인계해야 할 때 사용. | `skills/outsourcing-handoff` |
| parallel-work | 동시에 여러 작업을 병렬로 진행해야 할 때 작업을 분리(브랜치/워크트리/워크스페이스)하고 충돌과 문맥 혼선을 줄이는 워크플로를 제공하라. git worktree 또는 jj workspace로 분리 실행, 스트림별 계획/문서/이슈 연계, 병렬 작업 규칙 설정이 필요할 때 사용. | `skills/parallel-work` |
| partner-brief | 파트너십 논의에 필요한 핵심 정보를 정리해야 할 때 사용. | `skills/partner-brief` |
| plan-archive | 계획 문서를 아카이브하고 PLAN.md 인덱스를 정리하라. 완료된 계획을 정리해야 할 때 사용. | `skills/plan-archive` |
| policy-docs | 내부 정책 문서를 작성하거나 갱신해야 할 때 사용. | `skills/policy-docs` |
| postmortem | 장애/이슈 발생 후 원인과 재발 방지 계획을 정리해야 할 때 사용. | `skills/postmortem` |
| pre-work-plan | 작업 직전에 계획을 세우고 간단한 작업은 파일로, 복잡한 작업은 Linear 이슈로 관리하라. 계획 수립, 범위/리스크 확인, 작업 시작 전 체크리스트가 필요할 때 사용. | `skills/pre-work-plan` |
| pricing-faq | 가격/플랜 관련 FAQ를 정리하고 설명 문구를 작성해야 할 때 사용. | `skills/pricing-faq` |
| procurement-vendor | 외주/구매/벤더 계약 관리를 위한 산출물을 정리해야 할 때 사용. | `skills/procurement-vendor` |
...
| roadmap-planner | 제품 로드맵을 분기/우선순위 기준으로 정리해야 할 때 사용. | `skills/roadmap-planner` |
| role-dispatcher | 작업 성격에 맞는 역할을 빠르게 선택하고 해당 역할 스킬로 안내해야 할 때 사용. | `skills/role-dispatcher` |
| sales-bd | 제안서, 파트너십 문서, 세일즈 메시지를 정리하고 전달해야 할 때 사용. | `skills/sales-bd` |
| sales-proposal | 세일즈 제안서 구조와 핵심 메시지를 정리해야 할 때 사용. | `skills/sales-proposal` |
| security-compliance | 보안 점검, 권한 검토, 위협 모델링 등 안전성 관련 산출물을 정리해야 할 때 사용. | `skills/security-compliance` |
| security-review | 보안 체크리스트 기반으로 변경 사항을 점검하고 리스크를 요약해야 할 때 사용. | `skills/security-review` |
| seo-research | 키워드 조사, 검색 의도, 경쟁 콘텐츠 분석을 정리해야 할 때 사용. | `skills/seo-research` |
| skill-indexer | 저장소 내 스킬 목록을 SKILLS.md에 정리하라. 스킬 인덱스 문서가 필요할 때 사용. | `skills/skill-indexer` |
| skill-packager | 스킬을 패키징(.skill)하고 검증 결과를 요약하라. 배포 전 패키징이 필요할 때 사용. | `skills/skill-packager` |
| skill-release | 스킬 배포를 위해 검증, 패키징, 변경 로그 업데이트를 수행하라. 스킬 릴리즈 준비가 필요할 때 사용. | `skills/skill-release` |
| skill-template-sync | 스킬 템플릿을 동기화하고 중복을 줄여라. PRD/TRD/PLAN 템플릿을 일관되게 유지해야 할 때 사용. | `skills/skill-template-sync` |
| skill-validator | 저장소의 스킬 구조와 SKILL.md 프론트매터를 검사하고 누락/불일치를 리포트하라. 스킬 품질 점검이 필요할 때 사용. | `skills/skill-validator` |
| stakeholder-update | 주간/월간 진행 상황을 이해관계자에게 공유해야 할 때 사용. | `skills/stakeholder-update` |
| status-report | 프로젝트 진행 상태를 요약해 공유해야 할 때 사용. | `skills/status-report` |
| support-response | 고객 문의 응답과 처리 기준을 정리해야 할 때 사용. | `skills/support-response` |
| test-orchestrator | 테스트 전략과 실행 순서를 정의하고 테스트 범위를 정리해야 할 때 사용. | `skills/test-orchestrator` |
| test-report | 테스트 결과를 요약하고 이슈/리스크를 보고해야 할 때 사용. | `skills/test-report` |
| threat-model | 위협 모델을 정리하고 공격 표면과 완화책을 문서화해야 할 때 사용. | `skills/threat-model` |
